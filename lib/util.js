...
export function serializeString(s) {
    ...
    else if (s.length < 0x10000) return Buffer.concat([Buffer.from([253]), packUInt16LE(s.length), Buffer.from(s)]);
    else if (s.length < 0x100000000) return Buffer.concat([Buffer.from([254]), packUInt32LE(s.length), Buffer.from(s)]);
...
export function getKotoBlockSubsidy(nHeight) {
    ...
    const halvings = Math.floor(
        (nHeight - consensusParams.SubsidySlowStartShift()) / consensusParams.nSubsidyHalvingInterval,
    );
