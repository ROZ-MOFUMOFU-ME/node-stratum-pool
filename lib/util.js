...
exports.getVersionByte = function (addr) {
    var versionByte = bases58.decode(addr).slice(0, 1);
    return versionByte;
};
...
exports.kotoAddressToScript = function (addr) {
    let decoded;
    try {
        decoded = bases58.decode(addr);
    } catch (e) {
        console.error(`Base58 decode failed for ${addr}: ${e.message}`);
        throw new Error('Base58 decode failed');
    }

    if (decoded.length !== 25 && decoded.length !== 26) {
        console.error(`Invalid address length for ${addr}. Expected length: 25 or 26, but got: ${decoded.length}`);
...
exports.bignumFromBitsBuffer = function (bitsBuff) {
    ...
    var target = bigBits.multipliedBy(
        bignum(2).exponentiatedBy(
            bignum(8).multipliedBy(
                numBytes - 3
            )
        )
    );
...
exports.getKotoBlockSubsidy = function (nHeight) {
    const COIN = new bignum(100000000);
    let nSubsidy = COIN.multipliedBy(100);

    if (nHeight === 1) {
        nSubsidy = COIN.multipliedBy(3920000);
    ...
    if (nHeight < consensusParams.nSubsidySlowStartInterval / 2) {
        nSubsidy = nSubsidy.dividedBy(consensusParams.nSubsidySlowStartInterval);
        return nSubsidy.multipliedBy(nHeight).toNumber();
    } else if (nHeight < consensusParams.nSubsidySlowStartInterval) {
        nSubsidy = nSubsidy.dividedBy(consensusParams.nSubsidySlowStartInterval);
        return nSubsidy.multipliedBy(nHeight + 1).toNumber();
    }
    ...
    nSubsidy = nSubsidy.shiftedBy(-halvings);
    return nSubsidy.toNumber();
};
