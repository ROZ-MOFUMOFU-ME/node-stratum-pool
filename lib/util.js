 ...
 */
 ...
 exports.bignumFromBitsBuffer = function (bitsBuff) {
    ...
    var bigBits = new bignum(bitsBuff.slice(1).toString('hex'), 16);
    var target = bigBits.multipliedBy(
        new bignum(2).exponentiatedBy(
            new bignum(8).multipliedBy(
                numBytes - 3
            )
        )
    );
...
exports.getKotoBlockSubsidy = function (nHeight) {
    ...
    var nSubsidy = COIN.multiplier(100);

    if (nHeight == 1) {
        nSubsidy = COIN.multiplier(3920000);
    ...
    if (nHeight < consensusParams.nSubsidySlowStartInterval / 2) {
        ...
        return nSubsidy.multiplier(nHeight).toNumber();
    } else if (nHeight < consensusParams.nSubsidySlowStartInterval) {
        nSubsidy = nSubsidy.div(consensusParams.nSubsidySlowStartInterval);
        return nSubsidy.multiplier(nHeight + 1).toNumber();
    }
