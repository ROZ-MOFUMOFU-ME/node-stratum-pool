...
const BlockTemplate = function BlockTemplate(
    ...
    // PoW preimage. The daemon's serialized header (and serializeHeader) hold the
    // roots in canonical little-endian = the full byte reverse of the getblocktemplate
    // hex. To make the miner reconstruct exactly that, send the roots with their eight
    // uint32 words in REVERSE ORDER (bytes within each word unchanged): the miner's
    // per-word byteswap then yields the canonical bytes, so the miner, the pool's
    // share hash and the daemon's block hash all agree.
    const swapRootWordOrder = (hex) => {
