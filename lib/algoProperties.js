...
const algos = (global.algos = {
    ...
    vipstar: {
        hash() {
            return function (data) {
                // VIPSTARCOIN (qtum/HTMLcoin fork) validates PoW via the daemon's
                // CBlockHeader::GetHash = plain sha256d over the FULL 181-byte
                // serialized header (bitcoin80 + hashStateRoot + hashUTXORoot +
                // prevoutStake + the empty vchBlockSig length byte). The old
                // multiHashing.vipstar binding (cpuminer sha256d_181_swap) expects
                // pre-byteswapped getwork pdata, not the canonical wire bytes
                // serializeHeader produces, so every share read as shareDiff 0.
